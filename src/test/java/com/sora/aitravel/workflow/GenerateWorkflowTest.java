 ...
 * <p>
 * 按照"先查数据 → 再生成行程 → 再校验事实 → 再给用户确认"的核心流程，
 * 13 个节点串联 + 条件分支 + 逐日循环。
 * <p>
 * 当前为骨架版：各节点用 log.info 占位，条件分支的"不通过/不确认"路径
 * 保留路由能力但默认走正常路径校验通过。
 */
@Slf4j
...
public class GenerateWorkflowTest {
    ...
    void buildGraph() throws Exception {
        ...
        graph.addNode(REQ_LOAD, node_async(state -> {
        ...
        // ──── 2. TripSkeletonNode ────
        graph.addNode(SKELETON, node_async(state -> {
        ...
        graph.addNode(CITY_DATA, node_async(state -> {
        ...
        // ──── 4. DayStateInitNode ────
        graph.addNode(DAY_INIT, node_async(state -> {
        ...
        graph.addNode(DAY_CONTEXT, node_async(state -> {
        ...
        // ──── 6. DayQueryPlanNode ────
        graph.addNode(DAY_QUERY, node_async(state -> {
            int currentDay = state.value(KEY_CURRENT_DAY, 1);
        ...
        // ──── 7. DayDataFetchNode ────
        graph.addNode(DAY_FETCH, node_async(state -> {
            int currentDay = state.value(KEY_CURRENT_DAY, 1);
        ...
        // ──── 8. DayDataRankNode ────
        graph.addNode(DAY_RANK, node_async(state -> {
        ...
        graph.addNode(DAY_GENERATE, node_async(state -> {
            int currentDay = state.value(KEY_CURRENT_DAY, 1);
            log.info("[9/13] AI 基于真实数据生成第 {} 天行程...", currentDay);
            log.info("       AI 只能使用 scenicCandidates/foodCandidates/hotelContext/transportRoutes");
            log.info("       不编造具体地点、开放时间、门票价格");
            return Map.of();
        }));

        // ──── 10. DayPlanValidateNode ────
        graph.addNode(DAY_VALIDATE, node_async(state -> {
            int currentDay = state.value(KEY_CURRENT_DAY, 1);
        ...
        graph.addNode(DAY_REVIEW, node_async(state -> {
            int currentDay = state.value(KEY_CURRENT_DAY, 1);
        ...
        // ──── 12. DayLockNode ────
        graph.addNode(DAY_LOCK, node_async(state -> {
            int currentDay = state.value(KEY_CURRENT_DAY, 1);
            int totalDays = state.value(KEY_TOTAL_DAYS, 3);
        ...
        // ──── 13. TripSummaryNode ────
        graph.addNode(SUMMARY, node_async(state -> {
        ...
        graph.addConditionalEdges(DAY_VALIDATE,
            edge_async(state -> {
        ...
        graph.addConditionalEdges(DAY_REVIEW,
            edge_async(state -> {
        ...
        graph.addConditionalEdges(DAY_LOCK,
            edge_async(state -> {
     ...
     * 运行完整工作流——3天行程生成。
     * <p>
     * 预期输出：13 个节点按顺序执行，逐日循环 3 次，最后进入总览。
     */
    @Test
