...
public class AmapPoiTool {
    ...
    public AmapApiResp<List<Poi>> searchPoiText(
            @ToolParam(description = "搜索关键词，如：西湖、海底捞、酒店；keywords和types至少传一个", required = false) String keywords,
            @ToolParam(description = "POI分类编码，多个用|分隔，如060101表示风景名胜", required = false) String types,
            @ToolParam(description = "搜索城市/区划，城市名或adcode，如杭州市/330100", required = false) String region,
            @ToolParam(description = "是否仅限制当前城市搜索，true仅当前城市，false全国搜索", required = false) Boolean cityLimit
    ) {
