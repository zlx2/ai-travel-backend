 ...
 * <p>
 * Open-Meteo 是开源免费天气 API，无需 API Key。
...
public class WeatherTool {
    ...
    private static final String GEO_API = "https://geocoding-api.open-meteo.com/v1/search"; // 地理编码接口，把城市名转化成经纬度
    private static final String FORECAST_API = "https://api.open-meteo.com/v1/forecast"; // 天气预报接口，根据经纬度获取天气数据
    ...
    @Tool(description = "查询指定城市的实时天气和未来16天天气预报，包括温度、天气状况等。"
            + "当用户询问某个城市或目的地的天气、未来几天天气怎么样时调用此工具。")
    public String getWeather(
        ...
        try {
            ...
            // 第二步：获取 7 天预报（含当前天气）
            String forecastUrl = FORECAST_API
                    + "?latitude=" + lat                                    // 纬度参数
                    + "&longitude=" + lon                                   // 经度参数
            ...
            String response = HttpRequest.get(forecastUrl)
    ...
    private double[] geocode(String city) {
        try {
            ...
            String response = HttpRequest.get(GEO_API)
                    .form("name", city)
                    .form("count", 1)
                    .form("language", "zh")
                    .form("format", "json")
    ...
    private String parseForecast(String city, String jsonResponse) {
        try {
            ...
            if (current != null) {
                ...
                double feelsLike = current.getDouble("apparent_temperature");         // 体感温度（考虑湿度、风速等因素，单位：°C）
                ...
                String weatherDesc = WMO_CODES.getOrDefault(weatherCode, "未知");     // 根据WMO代码获取天气描述，默认为"未知"

                sb.append("当前实况：\n");
                sb.append("   天气：").append(weatherDesc).append("\n");
                sb.append("   温度：").append(String.format("%.0f", temp)).append("°C（体感 ").append(String.format("%.0f", feelsLike)).append("°C）\n");
            ...
            if (daily != null) {
                ...
                JSONArray timeArr = daily.getJSONArray("time");              // 日期数组，如 ["2026-06-23", "2026-06-24", ...]
                ...
                for (int i = 0; i < timeArr.size(); i++) {
                    ...
                    sb.append("      天气：").append(weatherDesc).append("\n");
                    sb.append("      温度：").append(String.format("%.0f", minTemp)).append("°C ~ ").append(String.format("%.0f", maxTemp)).append("°C\n");
                    if (precip > 0) {
                        sb.append("      降水：").append(String.format("%.1f", precip)).append(" mm\n");
                    }
                    sb.append("\n");
                }
            }
