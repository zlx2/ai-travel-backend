 ...
 * POI 搜索文档：https://lbs.amap.com/api/webservice/guide/api/search
 * 酒店类型编码：100100（宾馆酒店）
 * </p>
 */
@Component
public class HotelTool {
     ...
     */
    private static final Map<String, int[]> BRAND_PRICES = Map.ofEntries(
            // 经济连锁
    ...
    @Tool(description = "搜索指定城市的酒店信息，包括酒店名称、地址、联系电话、评分、价格区间等。"
            + "当用户询问某个城市住哪里、推荐酒店、酒店价格时调用此工具。")
    public String searchHotel(
        ...
        try {
            ...
            sb.append("【").append(city).append(" 酒店推荐】")
                    .append("（入住：").append(checkInDate).append("，离店：").append(checkOutDate)
                    .append("，共").append(nights).append("晚）\n\n");
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (prices != null) {
                    ...
                    sb.append("   预估总价：¥").append(minTotal).append(" ~ ¥").append(maxTotal)
                            .append("（").append(nights).append("晚）\n");
                }
                sb.append("\n");
            }
    ...
    private String estimatePrice(String name, String type, String city) {
        ...
        // 2. 通过名称关键词判断档次
        if (name.contains("五星级") || name.contains("豪华") || name.contains("度假别墅")
                || name.contains("庄园") || name.contains("行宫")) {
        ...
        if (name.contains("公寓") || name.contains("民宿") || name.contains("旅租")
                || name.contains("青旅") || name.contains("青年") || name.contains("自助")) {
    ...
    private int[] adjustByCity(int[] baseRange, String city) {
        ...
        // 一线/热门旅游城市价格上浮 30%-50%
        if ("北京".contains(city) || "上海".contains(city) || "深圳".contains(city)
                || "广州".contains(city) || "三亚".contains(city)) {
            return new int[]{(int) (baseRange[0] * 1.4), (int) (baseRange[1] * 1.4)};
        }
        // 新一线/热门城市上浮 15%-20%
        if ("成都".contains(city) || "杭州".contains(city) || "重庆".contains(city)
                || "南京".contains(city) || "武汉".contains(city) || "西安".contains(city)) {
