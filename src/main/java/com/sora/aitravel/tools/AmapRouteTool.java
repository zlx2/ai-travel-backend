...
public class AmapRouteTool {
    ...
    public AmapApiResp<Route> drivingRoute(
            @ToolParam(description = "起点经纬度，经度,纬度", required = true) String origin,
            @ToolParam(description = "终点经纬度，经度,纬度", required = true) String destination,
            @ToolParam(description = "路线策略：0推荐，1高速优先，2不走高速，3躲避拥堵，4少收费等", required = false) Integer strategy,
    ...
    public AmapApiResp<Route> transitRoute(
            @ToolParam(description = "起点经纬度，经度,纬度", required = true) String origin,
            @ToolParam(description = "终点经纬度，经度,纬度", required = true) String destination,
            @ToolParam(description = "起点城市名称或adcode", required = true) String city1,
            @ToolParam(description = "终点城市名称或adcode，跨城必填", required = true) String city2,
            @ToolParam(description = "换乘策略：0最快捷，1最少换乘，2最少步行，3不坐地铁等", required = false) Integer strategy,
        ...
        return amapService.transitRoute(origin, destination, city1, city2, strategy, alternativeRoute, nightflag);
    }
}
