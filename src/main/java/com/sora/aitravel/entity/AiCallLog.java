 ...
 * AI 调用日志实体类。
 *
 * <p>对应数据库表 {@code ai_call_log}，记录每次调用 AI 模型的详细日志，包括调用用户、 所属会话、场景、模型名称、请求摘要（脱敏后）、响应摘要、调用是否成功、错误信息
 * 以及耗时等。失败调用必须落库以便排查问题。请求和响应仅记录必要摘要并对敏感信息 进行脱敏处理，避免保存过大的完整 Prompt。
 *
 * <table border="1">
