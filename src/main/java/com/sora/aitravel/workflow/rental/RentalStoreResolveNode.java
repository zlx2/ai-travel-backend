 ...
 * 租车服务点解析节点。
 *
 * <p>该节点是工作流中的中间步骤：读取上下文中的目标地点、城市和取还车用途，调用 {@link RentalStoreService}
 * 解析一个推荐服务点，并写回上下文。它不直接调用高德 API，也不负责库存、价格或订单。
 */
@Component
public class RentalStoreResolveNode implements WorkflowNode<RentalWorkflowContext> {
