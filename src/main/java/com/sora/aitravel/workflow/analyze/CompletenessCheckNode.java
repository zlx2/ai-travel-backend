 ...
 * 则设置对应的错误状态或触发补充询问。
 *
 * <p>在整个工作流中的位置：流程第 3 步（信息提取之后，目的地推荐之前）。
 *
 * <p>输入：{@link AnalyzeWorkflowContext#rawModelResponse}（模型提取的结构化信息）。 输出：将完整性检查结果写入 {@link
...
public class CompletenessCheckNode implements WorkflowNode<AnalyzeWorkflowContext> {
    ...
    public void execute(AnalyzeWorkflowContext context) {
        ...
        if (isBlank(requirement.departure())) {
            context.setStatus("NEED_MORE_INFO");
            return;
        }
        ...
        boolean hasRoadTarget =
                !isBlank(requirement.destination())
                        || !isBlank(requirement.routeRegion())
                        || (requirement.routeCities() != null
                                && !requirement.routeCities().isEmpty());
        if (roadTrip && !hasRoadTarget) {
            context.setStatus("NEED_MORE_INFO");
            return;
        }
        if (!roadTrip && isBlank(requirement.destination())) {
            context.setStatus("NEED_DESTINATION_CHOICE");
            return;
        }
