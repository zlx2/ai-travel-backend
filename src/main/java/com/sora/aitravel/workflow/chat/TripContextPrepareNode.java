 ...
 * 行程上下文准备节点。
 * <p>
 * 实现 {@link WorkflowNode} 接口，是 {@link AiChatWorkflow} 工作流的第二个步骤。
 * 负责将从数据库加载的原始行程计划 JSON 进行格式化和剪裁，
 * 转换为适合作为 AI 模型输入上下文的格式。可能包括过滤无关字段、
 * 结构化摘要或按时间线整理行程安排等操作。
 * <p>
 * 在整个工作流中的位置：聊天流程第 2 步（上下文加载之后，Prompt 构建之前）。
 * <p>
 * 输入：{@link ChatWorkflowContext#tripPlanJson}（原始行程计划 JSON）。
 * 输出：处理后的行程上下文字符串（可直接嵌入 Prompt 的文本），
