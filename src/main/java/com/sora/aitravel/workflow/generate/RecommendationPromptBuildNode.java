 ...
 * 推荐上下文提示词构建节点。
 *
 * <p>将结构化的景点、美食、住宿和交通建议转换为模型可读文本。当前假数据生成节点也会使用该文本，
 * 后续接入真实模型时可直接放进 TripPlanGenerateNode 的 prompt。
 */
@Component
public class RecommendationPromptBuildNode implements WorkflowNode<GenerateWorkflowContext> {
