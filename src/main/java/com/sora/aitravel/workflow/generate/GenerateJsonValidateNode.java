 ...
 * 生成结果 JSON 校验节点。
 * <p>
 * 实现 {@link WorkflowNode} 接口，是 {@link TripGenerateWorkflow} 工作流的第三个步骤。
 * 负责验证 AI 模型返回的行程计划 JSON 是否符合预期结构，包括：
 * dailyPlans 数组长度是否与请求的天数一致、每日计划中是否包含必要字段、
 * JSON 格式是否合法等。
 * <p>
 * 在整个工作流中的位置：生成流程第 3 步（模型调用之后，修复节点之前）。
 * 若校验通过则后续修复节点自动跳过。
 * <p>
 * 输入：{@link GenerateWorkflowContext#rawModelResponse}（模型返回的原始 JSON）。
