 ...
 *
 * <ul>
 *   <li>根据每天最后景点的坐标，调用高德周边搜索 API 获取附近酒店列表
 *   <li>将酒店数据（名称、坐标、评分、价格估算）填充到 {@code DailyPlan.nearbyHotels}
 *   <li>将推荐酒店信息填充到时间线的 {@code STAY_AREA} 节点，供前端地图渲染
 * </ul>
 *
...
public class NearbyHotelService {
     ...
     *
     * <ol>
     *   <li>遍历每日行程，找到当天 order 最大的景点（即最后游览的景点）
     *   <li>以该景点坐标为圆心，调用高德周边搜索 API 获取附近酒店
     *   <li>将搜索结果（含名称、坐标、评分、价格估算）挂到 {@code dailyPlan.nearbyHotels}
     * </ol>
     *
    ...
    /**
     * 解析高德 API 返回的原始 JSON，转换为 NearbyHotel 对象列表。 高德返回结构：{ "status": "1", "pois": [ { "name":...,
     * "location":"lng,lat", ... } ] }
     */
    ...
    /** 找到当天行程中 order 最大的景点（即最后一个景点）。 酒店搜索以该景点为中心坐标。 */
    ...
    /** 从景点列表中提取城市名称（优先取第一个有效 city）。 用于限定酒店搜索的城市范围。 */
     ...
     *
     * <ol>
     *   <li>在 timeline 中查找第一个 {@code type="STAY_AREA"} 的节点
     *   <li>取 {@code nearbyHotels} 列表中的第一家酒店作为推荐酒店
     *   <li>将酒店名称设为节点标题，坐标赋给节点位置，价格生成展示文案
     *   <li>将整个酒店列表挂到节点的 {@code nearbyHotels} 字段，供前端按需消费
     * </ol>
     *
    ...
    /**
     * 将评分字符串安全地解析为 double 数值。 解析失败或输入为空时返回 0，不抛异常。
     *
     ...
     * 根据酒店名称判断是否为豪华酒店。
     *
     * <p>匹配关键词：度假、精品、国际、大酒店、威斯汀、希尔顿、万豪、香格里拉。 命中任意一个关键词即判定为豪华酒店。
     *
     * @param name 酒店名称
     * @return true 表示豪华酒店
     */
